...
{   
    ...
    Vector3f p = Vector3f(x + 0.5f, y + 0.5f, _v[0].z());    
    float z;

    for(int i = 0; i < 3; i++)
    {
      Vector3f vT, vP, result;
      if(i == 2)
      {
        vT = _v[2] - _v[0];
        vP = _v[2] - p; 
...
static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector3f* v)
{
    ...
    return std::tuple<float, float, float>(c1, c2, c3);//{c1,c2,c3};
}
...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    ...
    for (auto& i : ind)
    {
        std::cout << "vm:"<< view *model*to_vec4(buf[i[0]], 1.0f)<<'\n'<<"i[0]:"<< i[0] << '\n';
